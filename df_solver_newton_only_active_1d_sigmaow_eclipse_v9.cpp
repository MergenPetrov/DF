                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          const double df_minor_water_fraction_eps = 1.0e-2;
                          const double df_phase_superficial_flux_floor = 1.0e-14;
                          ...
                          const double oil_abs_before_threshold = fabs (oil_superficial_velocity_input_before_threshold);
                          const double water_abs_before_threshold = fabs (water_superficial_velocity_input_before_threshold);
                          const double liquid_abs_before_threshold = oil_abs_before_threshold + water_abs_before_threshold;
                          const double water_fraction_before_threshold =
                              (liquid_abs_before_threshold > df_phase_superficial_flux_floor)
                                ? water_abs_before_threshold / liquid_abs_before_threshold
                                : 0.0;
                          const double oil_fraction_before_threshold =
                              (liquid_abs_before_threshold > df_phase_superficial_flux_floor)
                                ? oil_abs_before_threshold / liquid_abs_before_threshold
                                : 0.0;
                          const bool df_water_abs_small_before_threshold =
                              water_abs_before_threshold <= df_minor_water_superficial_flux_eps
                              && water_abs_before_threshold > df_phase_superficial_flux_floor;
                          const bool df_water_rel_small_before_threshold =
                              water_fraction_before_threshold <= df_minor_water_fraction_eps;
                          const bool df_water_fold_to_oil_allowed =
                              df_water_abs_small_before_threshold
                              && df_water_rel_small_before_threshold
                              && oil_abs_before_threshold > df_phase_superficial_flux_floor;
                          bool df_water_thresholded_to_oil = false;

                          // Treat water as absent only if it is small in both senses:
                          //   1) absolutely small by superficial velocity, and
                          //   2) relatively small inside the liquid split.
                          // If |jsw| < 1e-5 but water is not a minor fraction of the liquid
                          // flow, do NOT fold it into oil; keep both liquid phases and let the
                          // mode selector use FIXED_OW_FLOW_SPLIT or coupled OW.
                          if (df_water_fold_to_oil_allowed)
                            {
                          ...
                          if (df_water_abs_small_before_threshold)
                            {
                              PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                   "MERGEN_DF_PHASE_CLASSIFY: Well {}: Seg_idx = {}, water_abs_eps = {}, water_rel_eps = {}, oil_in_before = {}, water_in_before = {}, liquid_abs_before = {}, oil_frac_before = {}, water_frac_before = {}, fold_allowed = {}, oil_in_after = {}, water_in_after = {}, action = {}\n",
                                   wcb_wis->get_well_name (), seg.wsn->wsn_index,
                                   df_minor_water_superficial_flux_eps,
                                   df_minor_water_fraction_eps,
                                   oil_superficial_velocity_input_before_threshold,
                                   water_superficial_velocity_input_before_threshold,
                                   liquid_abs_before_threshold,
                                   oil_fraction_before_threshold,
                                   water_fraction_before_threshold,
                                   df_water_fold_to_oil_allowed ? 1 : 0,
                                   oil_superficial_velocity_input, water_superficial_velocity_input,
                                   df_water_thresholded_to_oil ? "FOLD_WATER_TO_OIL" : "KEEP_WATER_ACTIVE_RELATIVE_NOT_SMALL");
                            }

                          const double liquid_superficial_velocity_input =
                              oil_superficial_velocity_input + water_superficial_velocity_input;
                          (void) liquid_superficial_velocity_input;
                          ...
                          auto evaluate_df_value_alphao = [&] (double alpha_g_trial,
                                                               ...
                                                               df_local_value_state_t &S)
                            {
                              ...
                              // ECLIPSE TDM Eq. 8.87: sigma_ow is the oil/water interfacial tension.
                              // If STOW is not supplied, ECLIPSE computes it internally as the
                              // difference between sigma_go and sigma_wg.  It is not beta-dependent.
                          ...
                          enum df_newton_mode_t
                            {
                              ...
                              DF_NM_OW_ONLY = 5         // 1D Newton: alpha_g = 0, solve oil/water split
                            };

                          const double newton_phase_flux_eps = 1.0e-14;
                          const double jo_abs_mode_for_select = fabs (oil_superficial_velocity_input);
                          const double jw_abs_mode_for_select = fabs (water_superficial_velocity_input);
                          const double jg_abs_mode_for_select = fabs (gas_superficial_velocity_input);
                          const double jl_abs_mode_for_select = jo_abs_mode_for_select + jw_abs_mode_for_select;
                          const double fo_flow_for_select =
                              (jl_abs_mode_for_select > newton_phase_flux_eps)
                                ? jo_abs_mode_for_select / jl_abs_mode_for_select
                                : 0.0;
                          const double fw_flow_for_select =
                              (jl_abs_mode_for_select > newton_phase_flux_eps)
                                ? jw_abs_mode_for_select / jl_abs_mode_for_select
                                : 0.0;
                          (void) fo_flow_for_select;
                          (void) fw_flow_for_select;
                          const bool newton_has_gas = jg_abs_mode_for_select > newton_phase_flux_eps;
                          const bool newton_has_oil = jo_abs_mode_for_select > newton_phase_flux_eps;
                          const bool newton_has_water = jw_abs_mode_for_select > newton_phase_flux_eps;
                          const bool newton_has_liquid = jl_abs_mode_for_select > newton_phase_flux_eps;

                          const double beta_flow =
                              (newton_has_liquid)
                                ? tnav_div (jo_abs_mode_for_select,
                                            std::max (newton_phase_flux_eps, jl_abs_mode_for_select))
                                : beta_input;

                          // FIXED_OW_SPLIT is now used only in the homogeneous oil/water limit.
                          // Do not select it merely because one superficial velocity is absolutely small.
                          const double ow_probe_ag = std::max (simplex_eps, std::min (1.0 - simplex_eps, alpha_g_newton));
                          const double ow_probe_ao = beta_flow * std::max (0.0, 1.0 - ow_probe_ag);
                          df_local_value_state_t ow_slip_probe_state;
                          evaluate_df_value_alphao (ow_probe_ag, ow_probe_ao, ow_slip_probe_state);
                          const double ow_velocity_slip_probe = fabs (ow_slip_probe_state.vels.oil - ow_slip_probe_state.vels.water);
                          const double ow_drift_abs_probe = fabs (ow_slip_probe_state.drift_velocity_OW);
                          const double ow_slip_velocity_tol = 1.0e-5;
                          const double ow_slip_drift_tol = 1.0e-5;
                          const bool ow_slip_degenerate =
                              (ow_velocity_slip_probe <= ow_slip_velocity_tol)
                              && (ow_drift_abs_probe <= ow_slip_drift_tol);

                          df_newton_mode_t df_newton_mode = DF_NM_GL_ONLY;
                          if (newton_has_liquid && newton_has_oil && newton_has_water)
                            {
                              if (!newton_has_gas)
                                df_newton_mode = DF_NM_OW_ONLY;
                              else if (ow_slip_degenerate)
                                df_newton_mode = DF_NM_FIXED_OW_SPLIT;
                              else
                          ...
                          // OW-only is gas-absent and solved by 1D Newton in alpha_o with alpha_g=0.
                          const bool newton_second_eq_is_water =
                              (df_newton_mode == DF_NM_COUPLED_OW || df_newton_mode == DF_NM_OW_ONLY)
                              && (fabs (water_superficial_velocity_input) > fabs (oil_superficial_velocity_input));
                              ...
                              switch (mode)
                                {
                                  ...
                                  case DF_NM_OW_ONLY:    return "OW_ONLY_GAS_ABSENT";
                                }
                              return "UNKNOWN";
                            };

                          PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                               "MERGEN_DF_MODE_SELECT: Well {}: Seg_idx = {}, selected_mode = {}, gas_abs = {}, oil_abs = {}, water_abs = {}, liquid_abs = {}, oil_frac = {}, water_frac = {}, beta_flow = {}, ow_slip_degenerate = {}, ow_velocity_slip_probe = {}, ow_drift_abs_probe = {}, ow_probe_alpha_g = {}, ow_probe_alpha_o = {}, ow_probe_alpha_w = {}\n",
                               wcb_wis->get_well_name (), seg.wsn->wsn_index,
                               df_newton_mode_name (df_newton_mode), jg_abs_mode_for_select,
                               jo_abs_mode_for_select, jw_abs_mode_for_select, jl_abs_mode_for_select,
                               fo_flow_for_select, fw_flow_for_select, beta_flow,
                               ow_slip_degenerate ? 1 : 0, ow_velocity_slip_probe, ow_drift_abs_probe,
                               ow_slip_probe_state.holdups.gas,
                               ow_slip_probe_state.holdups.oil,
                               ow_slip_probe_state.holdups.water);
                          ...
                          auto evaluate_newton_value = [&] (double alpha_g_trial,
                                                            ...
                                                            df_local_value_state_t &S)
                            {
                              ...
                              else if (mode == DF_NM_OW_ONLY)
                                {
                                  if (!std::isfinite (ag))
                                    ag = simplex_eps;
                                  ag = std::max (simplex_eps, std::min (1.0 - simplex_eps, ag));
                                  const double alpha_l_local = 1.0 - ag;
                                  if (!std::isfinite (ao))
                                    ao = beta_flow * alpha_l_local;
                                  ao = std::max (0.0, std::min (alpha_l_local, ao));
                                }
                          ...
                          auto compute_newton_residuals = [&] (double alpha_g_trial,
                                                               ...
                                                               double &res_norm_out)
                            {
                              ...
                              if (mode == DF_NM_OW_ONLY)
                                {
                                  F1_out = newton_second_eq_is_water ? Rw_out : Ro_out;
                                  F2_out = 0.0;
                                }
                              else
                                {
                          ...
                          if ((df_newton_mode == DF_NM_COUPLED_OW || df_newton_mode == DF_NM_OW_ONLY) && fabs (beta_newton_initial - beta_flow) > 0.75)
                            {
                          ...
                          else if (df_newton_mode == DF_NM_OW_ONLY)
                            {
                              alpha_g_newton_only = 0.0;
                              alpha_o_newton_only = std::max (0.0, std::min (1.0, beta_flow));
                            }
                          ...
                          for (int it_loc = 1; it_loc <= newton_max_it; ++it_loc)
                            {
                              ...
                              const double ag_upper = (df_newton_mode == DF_NM_COUPLED_OW || df_newton_mode == DF_NM_OW_ONLY)
                              ...
                              double J12 = 0.0;
                              double J22 = 1.0;
                              if (df_newton_mode == DF_NM_COUPLED_OW || df_newton_mode == DF_NM_OW_ONLY)
                                {
                                  const double alpha_l_for_ao = std::max (0.0, 1.0 - alpha_g_newton_only);
                                  const double ao_upper = (df_newton_mode == DF_NM_OW_ONLY) ? 1.0 : alpha_l_for_ao;
                              ...
                              const double detJ = (df_newton_mode == DF_NM_COUPLED_OW)
                                                    ? (J11 * J22 - J12 * J21)
                                                    : ((df_newton_mode == DF_NM_OW_ONLY) ? J12 : J11);
                              ...
                              for (int ils = 0; ils < 18; ++ils)
                                {
                                  ...
                                  if (df_newton_mode == DF_NM_OIL_ONLY)
                                    ...
                                    {
                                      ag_trial = 0.0;
                                      ao_trial = std::max (0.0, std::min (1.0, ao_trial));
                                    }
                                  else if (df_newton_mode == DF_NM_GL_ONLY || df_newton_mode == DF_NM_FIXED_OW_SPLIT)
                                    {
                                      ag_trial = std::max (simplex_eps, std::min (1.0 - simplex_eps, ag_trial));
                      ...
                      // ECLIPSE TDM Eq. 8.87: sigma_ow is the oil/water interfacial tension.
                      // If STOW is not supplied, it is calculated internally as the difference
                      // between sigma_go and sigma_wg from the gas/oil and gas/water correlations.
                      // It must not depend on beta_o.
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        {
                          ...
                          D_wat_oil_interfacial_tension_partial_D_seg_vars[id]
                              ...
                              * (D_sigma_go - D_sigma_gw);
                        }

                      const double D_wat_oil_interfacial_tension_D_beta_o = 0.0;
                      ...
                      // sigma_ow and Vc_OW are beta-independent in the ECLIPSE default OW model.
                      // Vd_OW still depends on beta_o through (1-beta_o)^2 below.
                      double D_bubble_rise_velocity_OW_D_beta_o = 0.0;
                      ...
                      const bool df_newton_ow_only_jacobian = (df_newton_mode == DF_NM_OW_ONLY);
                      ...
                      if (df_newton_mode == DF_NM_OW_ONLY)
                        {
                          J11 = 1.0;
                          J12 = 0.0;
                        }

                      if (!df_newton_uses_2d_ift && !df_newton_ow_only_jacobian)
                        {
                      ...
                      double detJ = df_newton_uses_2d_ift ? (J11 * J22 - J12 * J21)
                                                              : (df_newton_ow_only_jacobian ? J22 : J11);
                      PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                      ...
                      if (used_small_j_fallback)
                        {
                          ...
                          else if (df_newton_ow_only_jacobian)
                            {
                              J22 += jac_reg;
                              detJ = J22;
                            }
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        {
                          ...
                          const double R1_x = (df_newton_mode == DF_NM_OW_ONLY) ? 0.0 : Rg_x;

                          if (df_newton_uses_2d_ift)
                            {
                              D_alpha_g_D_seg_vars[id] =
                                  tnav_div (-J22 * R1_x + J12 * R2_x, detJ);

                              D_alpha_o_D_seg_vars[id] =
                                  tnav_div ( J21 * R1_x - J11 * R2_x, detJ);
                          ...
                          else if (df_newton_ow_only_jacobian)
                            {
                              ...
                              D_alpha_o_D_seg_vars[id] = tnav_div (-R2_x, detJ);
                              D_alpha_w_D_seg_vars[id] = -D_alpha_o_D_seg_vars[id];
                              if (ow_stage_active)
                                {
                                  D_beta_o_D_seg_vars[id] =
                                      tnav_div (D_alpha_o_D_seg_vars[id] * alpha_l - alpha_o * D_alpha_l_D_seg_vars[id],
                    ...
                    int &dbg_inner_it)
{
  ...
  const double df_minor_water_fraction_eps = 1.0e-2;
  const double df_phase_superficial_flux_floor = 1.0e-14;
  const double oil_abs_before_threshold = fabs (oil_superficial_velocity_input);
  const double water_abs_before_threshold = fabs (water_superficial_velocity_input);
  const double liquid_abs_before_threshold = oil_abs_before_threshold + water_abs_before_threshold;
  const double water_fraction_before_threshold =
      (liquid_abs_before_threshold > df_phase_superficial_flux_floor)
        ? water_abs_before_threshold / liquid_abs_before_threshold
  ...
  auto evaluate_df_value_alphao =
    [&] (double alpha_g_trial, double alpha_o_trial, df_local_value_state_t &S)
    {
      ...
      // ECLIPSE TDM Eq. 8.87: sigma_ow is beta-independent.
      // If STOW is not supplied, use |sigma_go - sigma_wg|.
      S.oil_water_interfacial_tension =
          fabs (gas_oil_interfacial_tension - gas_wat_interfacial_tension);
  ...
  const double ow_probe_ag = std::max (simplex_eps, std::min (1.0 - simplex_eps, alpha_g_solver_seed_raw));
  const double ow_probe_ao = beta_flow * std::max (0.0, 1.0 - ow_probe_ag);
  df_local_value_state_t ow_slip_probe_state;
  evaluate_df_value_alphao (ow_probe_ag, ow_probe_ao, ow_slip_probe_state);
  const double ow_velocity_slip_probe = fabs (ow_slip_probe_state.vels.oil - ow_slip_probe_state.vels.water);
