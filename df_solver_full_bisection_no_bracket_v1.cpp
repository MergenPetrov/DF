                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          auto choose_best_full_interval_candidate = [&] (const std::function<double(double)> &f,
                                                                 double x_left,
                                                                 double x_right,
                                                                 double x_seed,
                                                                 double &f_best) -> double
                            {
                              const double lo = std::min (x_left, x_right);
                              const double hi = std::max (x_left, x_right);
                              double x_best = x_left;
                              f_best = f (x_left);
                              double best_abs = std::isfinite (f_best) ? fabs (f_best) : 1.0e300;

                              const double f_right = f (x_right);
                              if (std::isfinite (f_right) && fabs (f_right) < best_abs)
                                {
                                  x_best = x_right;
                                  f_best = f_right;
                              ...
                              const double x_seed_clamped = std::max (lo, std::min (hi, x_seed));
                              const double f_seed = f (x_seed_clamped);
                              if (std::isfinite (f_seed) && fabs (f_seed) < best_abs)
                                {
                                  x_best = x_seed_clamped;
                                  f_best = f_seed;
                                }
                          ...
                          auto solve_scalar_root = [&] (const std::function<double(double)> &f,
                                                        ...
                                                        int &it_out) -> bool
                            {
                              ...
                              if (std::isfinite (fl) && fabs (fl) <= tol)
                                {
                                  x_out = xl;
                                  f_out = fl;
                                  if (iter_logger)
                              ...
                              if (std::isfinite (fr) && fabs (fr) <= tol)
                                {
                                  x_out = xr;
                                  f_out = fr;
                                  if (iter_logger)
                              ...
                              if (!(std::isfinite (fl) && std::isfinite (fr)) || fl * fr > 0.0)
                                {
                                  x_out = choose_best_full_interval_candidate (f, x_left, x_right, x_seed, f_out);
                                  if (iter_logger)
                                    iter_logger (it_out, x_out, f_out, xl, xr, fl, fr);
                                  return false;
                                }
                              ...
                              for (int it_loc = 0; it_loc < max_it_local; ++it_loc)
                                {
                                  ...
                                  if (iter_logger)
                                    iter_logger (it_out, xm, fm, xl, xr, fl, fr);

                                  if (!std::isfinite (fm))
                                    {
                                      x_out = choose_best_full_interval_candidate (f, xl, xr, x_seed, f_out);
                                      return false;
                                    }

                                  if (fabs (fm) <= tol || fabs (xr - xl) <= 1.0e-14)
                                    {
                                      x_out = xm;
                                      f_out = fm;
                                      return true;
                          ...
                          const double gl_left = 0.0;
                          const double gl_right = 1.0;
                          int gl_it = 0;
                          int ow_it = 0;

                          PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                               "MERGEN_DF_FULL_BISECTION: Well {}: Seg_idx = {}, stage = GL, xl = {}, xr = {}, seed = {}, direction = 0_TO_1, bracket_scan = 0, seed_from_prev_df = {}, seed_generation = {}\n",
                               wcb_wis->get_well_name (), seg.wsn->wsn_index,
                               gl_left, gl_right, alpha_g_gl,
                              ...
                              [&] (int it_loc, double x_value, double fx_value, double xl_value, double xr_value, double fl_value, double fr_value)
                                {
                                  ...
                                  log_df_iter_state ("GL_BISECTION_FULL", it_loc, x_value, fx_value, xl_value, xr_value, fl_value, fr_value, S_dbg);
                                };
                          solve_scalar_root ([&] (double ag) { return compute_rg_only (ag, nullptr); },
                                             gl_iter_logger,
                                             gl_left, gl_right, alpha_g_gl, 1.0e-12, 80,
                          ...
                          if (alpha_l_gl > 1.0e-10 && fabs (liquid_superficial_velocity_input) > 1.0e-14)
                            {
                              ...
                              const double beta_left = 1.0; // OW diagnostic run: start from oil-rich end.
                              const double beta_right = 0.0; // ...and bisect toward water-rich end.
                              const double Ro_beta_left = compute_ro_only (alpha_g_gl, beta_left, nullptr);
                              const double Ro_beta_right = compute_ro_only (alpha_g_gl, beta_right, nullptr);
                              const double Ro_target = compute_ro_only (alpha_g_gl, beta_target, nullptr);
                              PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                   "MERGEN_DF_FULL_BISECTION: Well {}: Seg_idx = {}, stage = OW, xl = {}, xr = {}, seed = {}, f_xl = {}, f_xr = {}, f_seed = {}, direction = 1_TO_0, bracket_scan = 0\n",
                                   wcb_wis->get_well_name (), seg.wsn->wsn_index,
                                   beta_left, beta_right, beta_target, Ro_beta_left, Ro_beta_right, Ro_target);
                                  ...
                                  [&] (int it_loc, double x_value, double fx_value, double xl_value, double xr_value, double fl_value, double fr_value)
                                    {
                                      ...
                                      log_df_iter_state ("OW_BISECTION_FULL", it_loc, x_value, fx_value, xl_value, xr_value, fl_value, fr_value, S_dbg);
                                    };
                              solve_scalar_root ([&] (double b) { return compute_ro_only (alpha_g_gl, b, nullptr); },
                                                 ow_iter_logger,
                                                 beta_left, beta_right, beta_target, 1.0e-12, 80,
                    ...
                    int &dbg_inner_it)
{
  ...
  if (alpha_l_gl > 1.0e-10 && fabs (liquid_superficial_velocity_input) > 1.0e-14)
    {
      const double beta_target = clamp01 (beta_solver_seed);
      const double beta_left = 1.0;
      const double beta_right = 0.0;
      double Ro_stage = 0.0;
      solve_scalar_root ([&] (double b) { return compute_ro_only (alpha_g_gl, b, nullptr); },
                         beta_left, beta_right, beta_target, 1.0e-12, 80,
                         beta_ow, Ro_stage, ow_it);
