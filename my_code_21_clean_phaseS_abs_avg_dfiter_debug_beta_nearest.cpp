if (element_status)
        {
          ...
          // Clean DF mode: initialize holdups only from the current flash phase_S.
                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          const double average_volumetric_mixture_velocity_sign =
                              (average_volumetric_mixture_velocity_raw > 0.0)
                                ? 1.0
                                : ((average_volumetric_mixture_velocity_raw < 0.0) ? -1.0 : 0.0);
                          double average_volumetric_mixture_velocity = fabs (average_volumetric_mixture_velocity_raw);
                              ...
                              for (unsigned int id = 0; id < 1U + mp.nc + 1U; id++)
                                {
                                  ...
                                  wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[id] =
                                      average_volumetric_mixture_velocity_sign * D_average_volumetric_mixture_velocity_raw;
                                }
                          ...
                          double gas_superficial_velocity_input = alpha_g_seed_raw * mixture_superficial_velocity;
                          double oil_superficial_velocity_input = alpha_o_seed_raw * mixture_superficial_velocity;
                          double water_superficial_velocity_input = alpha_w_seed_raw * mixture_superficial_velocity;
                          ...
                          int gl_it = 0;
                          int ow_it = 0;
                          const bool gl_root_exists = find_first_root_bracket (
                              [&] (double ag) { return compute_rg_only (ag, nullptr); },
                              simplex_eps, 1.0 - simplex_eps, 64,
                              gl_left, gl_right, gl_best, gl_best_res);
...
// Export final value-path scalars for logs / FD debug / post-processing.
                          ...
                          bool gas_input_sign_mismatch_dbg = false;
                          {
                            ...
                            gas_input_sign_mismatch_dbg =
                                (gas_superficial_velocity_input * gas_velocity_at_alpha_floor_dbg < 0.0);
                          }
                          ...
                          const double dbg_Rg_iter = new_holdups.gas   * new_vels.gas   - gas_superficial_velocity_input;
                          const double dbg_Ro_iter = new_holdups.oil   * new_vels.oil   - oil_superficial_velocity_input;
                          const double dbg_Rw_iter = new_holdups.water * new_vels.water - water_superficial_velocity_input;
                          PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
        ...
        if (same_active_set && inner_converged && same_flash_phase_set && seg.wsn->wsn_index != TOP_SEG_INDEX)
          {
            ...
            bool applied_phase_boundary_override = true;
            ...
            if (applied_phase_boundary_override)
              {
                D_alpha_g_D_seg_vars = D_alpha_g_patched;
                D_beta_o_D_seg_vars = D_beta_o_patched;
                D_alpha_o_D_seg_vars = D_alpha_o_patched;
                D_alpha_w_D_seg_vars = D_alpha_w_patched;
                D_alpha_l_D_seg_vars = D_alpha_l_patched;
                D_rho_avg_D_seg_vars = D_rho_avg_patched;

                for (unsigned int id = 0; id < nseg_vars; ++id)
                  {
                    wsncs->D_rho_avg_D_seg_vars[id] = D_rho_avg_patched[id];
                    wsncs->D_phase_holdup_D_seg_vars[mp.np * id + PHASE_GAS] = D_alpha_g_patched[id];
                    wsncs->D_phase_holdup_D_seg_vars[mp.np * id + PHASE_OIL] = D_alpha_o_patched[id];
                    wsncs->D_phase_holdup_D_seg_vars[mp.np * id + PHASE_WATER] = D_alpha_w_patched[id];
                  }

                for (unsigned int id = 0; id < 1U + mp.nc; ++id)
                  {
                    wsncs->phase_D_S[id * mp.np + PHASE_GAS]   = D_alpha_g_patched[id];
                    wsncs->phase_D_S[id * mp.np + PHASE_OIL]   = D_alpha_o_patched[id];
                    wsncs->phase_D_S[id * mp.np + PHASE_WATER] = D_alpha_w_patched[id];

                    wsncs->avg_D_xi[id] =
                          element_status->phase_xi[PHASE_GAS]   * D_alpha_g_patched[id]
                        + element_status->phase_xi[PHASE_OIL]   * D_alpha_o_patched[id]
                        + element_status->phase_xi[PHASE_WATER] * D_alpha_w_patched[id]
                        + alpha_g * element_status->phase_D_xi[id * mp.np + PHASE_GAS]
                        + alpha_o * element_status->phase_D_xi[id * mp.np + PHASE_OIL]
                        + alpha_w * element_status->phase_D_xi[id * mp.np + PHASE_WATER];

                    wsncs->avg_D_rho[id] = D_rho_avg_patched[id];
                  }
                ...
                PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                     "Seg {} applied phase-boundary one-sided overrides to exported DF derivatives\n",
                     seg.wsn->wsn_index);
              }
          }
                          }
                    ...
                    int &dbg_inner_it)
{
  ...
  // Clean DF mode: initialization is taken only from current flash phase_S.
  const bool use_upwind_state_inside_df = false;
  if (use_upwind_state_inside_df
      && wsncs->wsn_flow_dir == segment_flow_direction_t::from_parent_to_child)
    {
  ...
  average_volumetric_velocity = fabs (average_volumetric_velocity_raw);
