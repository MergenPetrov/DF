                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          double alpha_g_gl = std::max (simplex_eps, std::min (1.0 - simplex_eps, alpha_g_newton));
                          double beta_ow = clamp01 (beta_solver_seed);
                          ...
                          // -----------------------------------------------------------------------------
                          // Stage-1 BRACKET solve is kept as a robust continuation/initial-guess generator.
                          // The final local state is then corrected by the coupled 2x2 Newton below.
                          // -----------------------------------------------------------------------------
                          ...
                          double Rg_stage = 0.0;
                          if (!gl_brackets.empty ())
                            {
                              const int ibest = select_nearest_root_bracket (gl_brackets, alpha_g_gl);
                              gl_left = gl_brackets[ibest].xl;
                              gl_right = gl_brackets[ibest].xr;
                              PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                   "MERGEN_DF_BRACKET_SELECT: Well {}: Seg_idx = {}, stage = GL, idx = {}, xl = {}, xr = {}, target = {}, seed_from_prev_df = {}, seed_generation = {}\n",
                                   wcb_wis->get_well_name (), seg.wsn->wsn_index, ibest, gl_left, gl_right, alpha_g_gl,
                              ...
                              solve_scalar_root ([&] (double ag) { return compute_rg_only (ag, nullptr); },
                                                 gl_iter_logger,
                                                 gl_left, gl_right, alpha_g_gl, 1.0e-12, 32,
                          ...
                          if (alpha_l_gl > 1.0e-10 && fabs (liquid_superficial_velocity_input) > 1.0e-14)
                            {
                              const double beta_target = clamp01 (beta_solver_seed);
                          ...
                          df_local_value_state_t state_staged;
                          evaluate_ow_value (alpha_g_gl, beta_ow, state_staged);

                          double Rg_staged = 0.0;
                          double Ro_staged = 0.0;
                          double Rw_staged = 0.0;
                          double res_norm_staged = 0.0;
                          compute_full_residuals (state_staged, Rg_staged, Ro_staged, Rw_staged, res_norm_staged);

                          // -----------------------------------------------------------------------------
                          // Stage-2 coupled local Newton solve in variables (alpha_g, beta_o).
                          // This is the actual final local DF solve.  It removes the inconsistency where
                          // OW changed beta_o after GL had already solved Rg(alpha_g, beta_old)=0.
                          // -----------------------------------------------------------------------------
                          auto evaluate_coupled_value = [&] (double alpha_g_trial,
                                                             double beta_trial,
                                                             df_local_value_state_t &S)
                            {
                              const double alpha_g_clamped = std::max (simplex_eps,
                                                                       std::min (1.0 - simplex_eps, alpha_g_trial));
                              const double beta_clamped = std::max (beta_min, std::min (beta_max, beta_trial));
                              const double alpha_o_trial = beta_clamped * (1.0 - alpha_g_clamped);
                              evaluate_df_value_alphao (alpha_g_clamped, alpha_o_trial, S);
                            };

                          auto compute_coupled_residuals = [&] (double alpha_g_trial,
                                                               double beta_trial,
                            ...
                            {
                              df_local_value_state_t S_tmp;
                              evaluate_coupled_value (alpha_g_trial, beta_trial, S_tmp);
                              compute_full_residuals (S_tmp, Rg_out, Ro_out, Rw_out, res_norm_out);
                              if (S_out)
                                *S_out = S_tmp;
                            };

                          const double coupled_flux_scale =
                              std::max (1.0e-14,
                                        fabs (gas_superficial_velocity_input)
                                        + fabs (oil_superficial_velocity_input)
                                        + fabs (water_superficial_velocity_input));

                          auto coupled_norm2 = [&] (double Rg, double Ro) -> double
                            {
                              return std::sqrt (Rg * Rg + Ro * Ro) / coupled_flux_scale;
                            };

                          double alpha_g_cpl = std::max (simplex_eps, std::min (1.0 - simplex_eps, alpha_g_gl));
                          double beta_cpl = std::max (beta_min, std::min (beta_max, beta_ow));
                          df_local_value_state_t state_now;
                          evaluate_coupled_value (alpha_g_cpl, beta_cpl, state_now);

                          double Rg_cpl = 0.0;
                          double Ro_cpl = 0.0;
                          double Rw_cpl = 0.0;
                          double res_norm_cpl = 0.0;
                          compute_coupled_residuals (alpha_g_cpl, beta_cpl, &state_now, Rg_cpl, Ro_cpl, Rw_cpl, res_norm_cpl);
                          double coupled_norm = coupled_norm2 (Rg_cpl, Ro_cpl);

                          const double staged_norm2 = coupled_norm2 (Rg_staged, Ro_staged);
                          const bool coupled_newton_attempted =
                              (std::isfinite (coupled_norm)
                               && alpha_l_gl > 1.0e-10
                               && fabs (liquid_superficial_velocity_input) > 1.0e-14
                               && fabs (oil_superficial_velocity_input) > 1.0e-14
                               && fabs (water_superficial_velocity_input) > 1.0e-14);

                          bool coupled_newton_converged = false;
                          bool coupled_newton_used = false;
                          bool coupled_newton_fallback = false;
                          int coupled_it = 0;
                          double coupled_detJ_last = 0.0;
                          double coupled_step_last = 0.0;
                          double coupled_norm_initial = coupled_norm;
                          double coupled_norm_best = coupled_norm;
                          double alpha_g_best_cpl = alpha_g_cpl;
                          double beta_best_cpl = beta_cpl;
                          df_local_value_state_t state_best_cpl = state_now;

                          if (coupled_newton_attempted)
                            {
                              PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                   "MERGEN_DF_COUPLED_NEWTON_START: Well {}: Seg_idx = {}, alpha_g0 = {}, beta_o0 = {}, staged_Rg = {}, staged_Ro = {}, staged_Rw = {}, staged_norm2 = {}, flux_scale = {}, seed_from_prev_df = {}, seed_generation = {}\n",
                                   wcb_wis->get_well_name (), seg.wsn->wsn_index,
                                   alpha_g_cpl, beta_cpl, Rg_staged, Ro_staged, Rw_staged, staged_norm2, coupled_flux_scale,
                                   df_holdup_seed_from_lagged_map ? 1 : 0, static_cast<unsigned long> (df_holdup_seed_generation));

                              const int coupled_max_it = 20;
                              const double coupled_tol = 1.0e-10;
                              for (int it_cpl = 1; it_cpl <= coupled_max_it; ++it_cpl)
                                {
                                  coupled_it = it_cpl;
                                  compute_coupled_residuals (alpha_g_cpl, beta_cpl, &state_now, Rg_cpl, Ro_cpl, Rw_cpl, res_norm_cpl);
                                  coupled_norm = coupled_norm2 (Rg_cpl, Ro_cpl);

                                  if (std::isfinite (coupled_norm) && coupled_norm < coupled_norm_best)
                                    {
                                      coupled_norm_best = coupled_norm;
                                      alpha_g_best_cpl = alpha_g_cpl;
                                      beta_best_cpl = beta_cpl;
                                      state_best_cpl = state_now;
                                    }

                                  if (std::isfinite (coupled_norm) && coupled_norm <= coupled_tol)
                                    {
                                      coupled_newton_converged = true;
                                      coupled_newton_used = true;
                                      PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                           "MERGEN_DF_COUPLED_NEWTON: Well {}: Seg_idx = {}, it_loc = {}, accepted = {}, alpha_g = {}, beta_o = {}, alpha_o = {}, alpha_w = {}, Rg = {}, Ro = {}, Rw = {}, norm2 = {}, detJ = {}, step = {}, reason = CONVERGED\n",
                                           wcb_wis->get_well_name (), seg.wsn->wsn_index, it_cpl, 1,
                                           state_now.holdups.gas,
                                           (state_now.holdups.liquid > df_den_eps) ? state_now.holdups.oil / state_now.holdups.liquid : 0.0,
                                           state_now.holdups.oil, state_now.holdups.water,
                                           Rg_cpl, Ro_cpl, Rw_cpl, coupled_norm, coupled_detJ_last, coupled_step_last);
                                      break;
                                    }

                                  const double h_ag_base = 1.0e-6;
                                  const double h_be_base = 1.0e-6;
                                  const double ag_p = std::min (1.0 - simplex_eps, alpha_g_cpl + h_ag_base);
                                  const double ag_m = std::max (simplex_eps, alpha_g_cpl - h_ag_base);
                                  const double be_p = std::min (beta_max, beta_cpl + h_be_base);
                                  const double be_m = std::max (beta_min, beta_cpl - h_be_base);

                                  double Rg_ap = 0.0, Ro_ap = 0.0, Rw_dummy = 0.0, norm_dummy = 0.0;
                                  double Rg_am = 0.0, Ro_am = 0.0;
                                  double Rg_bp = 0.0, Ro_bp = 0.0;
                                  double Rg_bm = 0.0, Ro_bm = 0.0;
                                  compute_coupled_residuals (ag_p, beta_cpl, nullptr, Rg_ap, Ro_ap, Rw_dummy, norm_dummy);
                                  compute_coupled_residuals (ag_m, beta_cpl, nullptr, Rg_am, Ro_am, Rw_dummy, norm_dummy);
                                  compute_coupled_residuals (alpha_g_cpl, be_p, nullptr, Rg_bp, Ro_bp, Rw_dummy, norm_dummy);
                                  compute_coupled_residuals (alpha_g_cpl, be_m, nullptr, Rg_bm, Ro_bm, Rw_dummy, norm_dummy);

                                  const double dag_den = std::max (ag_p - ag_m, 1.0e-14);
                                  const double db_den = std::max (be_p - be_m, 1.0e-14);
                                  const double J11_cpl = (Rg_ap - Rg_am) / dag_den;
                                  const double J21_cpl = (Ro_ap - Ro_am) / dag_den;
                                  const double J12_cpl = (Rg_bp - Rg_bm) / db_den;
                                  const double J22_cpl = (Ro_bp - Ro_bm) / db_den;
                                  const double detJ_cpl = J11_cpl * J22_cpl - J12_cpl * J21_cpl;
                                  coupled_detJ_last = detJ_cpl;

                                  double d_ag = 0.0;
                                  double d_be = 0.0;
                                  bool linear_ok = std::isfinite (detJ_cpl) && fabs (detJ_cpl) > 1.0e-20;
                                  if (linear_ok)
                                    {
                                      d_ag = (-J22_cpl * Rg_cpl + J12_cpl * Ro_cpl) / detJ_cpl;
                                      d_be = ( J21_cpl * Rg_cpl - J11_cpl * Ro_cpl) / detJ_cpl;
                                      linear_ok = std::isfinite (d_ag) && std::isfinite (d_be);
                                    }

                                  if (!linear_ok)
                                    {
                                      coupled_newton_fallback = true;
                                      PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                           "MERGEN_DF_COUPLED_NEWTON: Well {}: Seg_idx = {}, it_loc = {}, accepted = {}, alpha_g = {}, beta_o = {}, alpha_o = {}, alpha_w = {}, Rg = {}, Ro = {}, Rw = {}, norm2 = {}, detJ = {}, J11 = {}, J12 = {}, J21 = {}, J22 = {}, reason = SINGULAR_LOCAL_JAC\n",
                                           wcb_wis->get_well_name (), seg.wsn->wsn_index, it_cpl, 0,
                                           state_now.holdups.gas,
                                           (state_now.holdups.liquid > df_den_eps) ? state_now.holdups.oil / state_now.holdups.liquid : 0.0,
                                           state_now.holdups.oil, state_now.holdups.water,
                                           Rg_cpl, Ro_cpl, Rw_cpl, coupled_norm, detJ_cpl, J11_cpl, J12_cpl, J21_cpl, J22_cpl);
                                      break;
                                    }

                                  bool accepted = false;
                                  double accepted_step = 0.0;
                                  double trial_norm = coupled_norm;
                                  double trial_Rg = Rg_cpl;
                                  double trial_Ro = Ro_cpl;
                                  double trial_Rw = Rw_cpl;
                                  df_local_value_state_t trial_state = state_now;
                                  for (int ils = 0; ils < 16; ++ils)
                                    {
                                      const double step = tnav_pow (0.5, ils);
                                      const double ag_trial = std::max (simplex_eps,
                                                                       std::min (1.0 - simplex_eps, alpha_g_cpl + step * d_ag));
                                      const double be_trial = std::max (beta_min,
                                                                       std::min (beta_max, beta_cpl + step * d_be));
                                      compute_coupled_residuals (ag_trial, be_trial, &trial_state,
                                                                 trial_Rg, trial_Ro, trial_Rw, norm_dummy);
                                      trial_norm = coupled_norm2 (trial_Rg, trial_Ro);
                                      if (std::isfinite (trial_norm)
                                          && (trial_norm < coupled_norm || trial_norm <= coupled_tol))
                                        {
                                          alpha_g_cpl = ag_trial;
                                          beta_cpl = be_trial;
                                          accepted = true;
                                          accepted_step = step;
                                          break;
                                  ...
                                  coupled_step_last = accepted_step;
                                  PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                                       "MERGEN_DF_COUPLED_NEWTON: Well {}: Seg_idx = {}, it_loc = {}, accepted = {}, alpha_g = {}, beta_o = {}, alpha_o = {}, alpha_w = {}, Rg = {}, Ro = {}, Rw = {}, norm2 = {}, trial_norm2 = {}, detJ = {}, J11 = {}, J12 = {}, J21 = {}, J22 = {}, d_alpha_g = {}, d_beta_o = {}, step = {}, vg = {}, vo = {}, vw = {}, C0 = {}, Vd = {}, C0_OW = {}, Vd_OW = {}\n",
                                       wcb_wis->get_well_name (), seg.wsn->wsn_index, it_cpl, accepted ? 1 : 0,
                                       state_now.holdups.gas,
                                       (state_now.holdups.liquid > df_den_eps) ? state_now.holdups.oil / state_now.holdups.liquid : 0.0,
                                       state_now.holdups.oil, state_now.holdups.water,
                                       Rg_cpl, Ro_cpl, Rw_cpl, coupled_norm, trial_norm,
                                       detJ_cpl, J11_cpl, J12_cpl, J21_cpl, J22_cpl,
                                       d_ag, d_be, accepted_step,
                              ...
                              if (coupled_newton_converged || coupled_norm_best <= std::max (1.0e-10, 0.1 * std::max (staged_norm2, 1.0e-30)))
                                {
                                  alpha_g_cpl = alpha_g_best_cpl;
                                  beta_cpl = beta_best_cpl;
                                  state_now = state_best_cpl;
                                  coupled_newton_used = true;
                                }
                              else
                                {
                                  state_now = state_staged;
                                  coupled_newton_fallback = true;
                                }
                            }
                          else
                          ...
                          PR1 (LOG_WELL_SECTION, LOG_MESSAGE, WELL_PARAMS,
                               "MERGEN_DF_COUPLED_NEWTON_FINAL: Well {}: Seg_idx = {}, attempted = {}, used = {}, converged = {}, fallback = {}, it = {}, alpha_g = {}, beta_o = {}, alpha_o = {}, alpha_w = {}, Rg = {}, Ro = {}, Rw = {}, res_norm = {}, norm2_initial = {}, norm2_best = {}, norm2_staged = {}, detJ_last = {}\n",
                               wcb_wis->get_well_name (), seg.wsn->wsn_index,
                               coupled_newton_attempted ? 1 : 0, coupled_newton_used ? 1 : 0,
                               coupled_newton_converged ? 1 : 0, coupled_newton_fallback ? 1 : 0,
                               coupled_it,
                               state_now.holdups.gas,
                               (state_now.holdups.liquid > df_den_eps) ? state_now.holdups.oil / state_now.holdups.liquid : 0.0,
                               state_now.holdups.oil, state_now.holdups.water,
                               Rg_final, Ro_final, Rw_final, res_norm_final,
                               coupled_norm_initial, coupled_norm_best, staged_norm2, coupled_detJ_last);

                          error = res_norm_final;
                          it = gl_it + ow_it + coupled_it;
                    ...
                    int &dbg_inner_it)
{
  ...
  double alpha_g_gl = std::max (simplex_eps, std::min (1.0 - simplex_eps, alpha_g_solver_seed_raw));
  double beta_ow = clamp01 (beta_solver_seed);
  double gl_best = alpha_g_gl;
  double gl_best_res = 0.0;
  double gl_left = simplex_eps;
  ...
  // Same local solver as production path, but without verbose PR1 diagnostics:
  // BRACKET staged solve only initializes the final coupled 2x2 Newton solve.
  std::vector<df_root_bracket_t> gl_brackets;
  collect_root_brackets (
      [&] (double ag) { return compute_rg_only (ag, nullptr); },
      simplex_eps, 1.0 - simplex_eps, 64,
  ...
  double Rw_staged = 0.0;
  double res_norm_staged = 0.0;
  compute_residuals_alphao (state_staged, Rg_staged, Ro_staged, Rw_staged, res_norm_staged);
  ...
  auto coupled_norm2 = [&] (double Rg, double Ro) -> double
    {
      return sqrt (Rg * Rg + Ro * Ro) / coupled_flux_scale;
    };
      ...
      for (int it_cpl = 1; it_cpl <= coupled_max_it; ++it_cpl)
        {
          ...
          if (!(std::isfinite (detJ_cpl) && fabs (detJ_cpl) > 1.0e-20))
            break;

          const double d_ag = (-J22_cpl * Rg_cpl + J12_cpl * Ro_cpl) / detJ_cpl;
          const double d_be = ( J21_cpl * Rg_cpl - J11_cpl * Ro_cpl) / detJ_cpl;
          if (!(std::isfinite (d_ag) && std::isfinite (d_be)))
            break;
          ...
          for (int ils = 0; ils < 16; ++ils)
            {
              ...
              compute_coupled_residuals (ag_trial, be_trial, &trial_state,
                                         trial_Rg, trial_Ro, trial_Rw, norm_dummy);
              trial_norm = coupled_norm2 (trial_Rg, trial_Ro);
              if (std::isfinite (trial_norm) && (trial_norm < coupled_norm || trial_norm <= coupled_tol))
                {
                  alpha_g_cpl = ag_trial;
                  beta_cpl = be_trial;
                  accepted = true;
  ...
  int it = gl_it + ow_it + coupled_it;
  dbg_active_flags |= coupled_newton_attempted ? (1u << 15) : 0u;
  dbg_active_flags |= coupled_newton_converged ? (1u << 16) : 0u;
  dbg_inner_it = it;
