                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          const double alpha_g_flash_init = 0;//element_status->phase_S[PHASE_GAS];
                          const double alpha_o_flash_init = 1;//element_status->phase_S[PHASE_OIL];
                          const double alpha_w_flash_init = 0;//element_status->phase_S[PHASE_WATER];

                          const double alpha_g_seed = use_previous_df_holdup_seed ? prev_alpha_g_state : alpha_g_flash_init;
                          const double alpha_o_seed = use_previous_df_holdup_seed ? prev_alpha_o_state : alpha_o_flash_init;
                          const double alpha_w_seed = use_previous_df_holdup_seed ? prev_alpha_w_state : alpha_w_flash_init;
                              ...
                              for (unsigned int id = 0; id < nseg_vars; ++id)
                                {
                                  if (id < 1U + mp.nc)
                                    {
                                      double D_Sg0 = element_status->phase_D_S[id * mp.np + PHASE_GAS];
                                      double D_So0 = element_status->phase_D_S[id * mp.np + PHASE_OIL];
                                      double D_Sw0 = element_status->phase_D_S[id * mp.np + PHASE_WATER];
                                      double D_alpha_l0 = D_So0 + D_Sw0;
                                      ...
                                      D_alpha_g_iter[id] = D_Sg0;
                                      if (fabs (alpha_l_init) > df_den_eps)
                                        {
                                          D_beta_o_iter[id] =
                                              (D_So0 * alpha_l_init - alpha_o_flash_init * D_alpha_l0)
                                              / (alpha_l_init * alpha_l_init);
                                        }
                                    }
                                }
                            }
                          ...
                          if (!use_previous_df_holdup_seed)
                            {
                              for (unsigned int id = 0; id < nseg_vars; ++id)
                                {
                                  double D_Sg = (id < 1U + mp.nc) ? element_status->phase_D_S[id * mp.np + PHASE_GAS]   : 0.;
                                  double D_So = (id < 1U + mp.nc) ? element_status->phase_D_S[id * mp.np + PHASE_OIL]   : 0.;
                                  double D_Sw = (id < 1U + mp.nc) ? element_status->phase_D_S[id * mp.np + PHASE_WATER] : 0.;
                                  ...
                                  D_gas_superficial_velocity_input_D_seg_vars[id] =
                                      D_Sg * mixture_superficial_velocity + alpha_g_flash_init * D_mixture_superficial_velocity_D_seg_vars[id];
                                  D_oil_superficial_velocity_input_D_seg_vars[id] =
                                      D_So * mixture_superficial_velocity + alpha_o_flash_init * D_mixture_superficial_velocity_D_seg_vars[id];
                                  D_water_superficial_velocity_input_D_seg_vars[id] =
                                      D_Sw * mixture_superficial_velocity + alpha_w_flash_init * D_mixture_superficial_velocity_D_seg_vars[id];
                                }
                            }
                          else
                            {
                          ...
                          while (error > 1.e-4 && it < max_it)
                            {
                             ...
                             double drift_velocity = tnav_div ((1 - temp_holdups.gas * C_0) * C_0 * K_g * bubble_rise_velocity,
                                                                sqrt (tnav_div (gas_density, liq_density)) * temp_holdups.gas * C_0 + 1 - temp_holdups.gas * C_0);
                             ...
                             wsncs->wsn_drift_velocity = drift_velocity;
                              ...
                              for (unsigned int id = 0; id < nseg_vars; ++id)
                                {
                                  ...
                                  const double sqrt_rg_rl = sqrt (tnav_div (gas_density, liq_density));
                                  const double D_sqrt_rg_rl =
                                      0.5 / df_safe_nonzero (sqrt_rg_rl)
                                      * (D_rho_g_x * liq_density - gas_density * D_liq_density_total)
                                      * tnav_div (1., liq_density * liq_density);
                                  ...
                                  const double den_vd = sqrt_rg_rl * temp_holdups.gas * C_0 + 1.0 - temp_holdups.gas * C_0;
                                  const double D_den_vd =
                                      D_sqrt_rg_rl * temp_holdups.gas * C_0
                                      + sqrt_rg_rl * (D_alpha_iter * C_0 + temp_holdups.gas * D_C0_total)
                                      - D_alpha_iter * C_0 - temp_holdups.gas * D_C0_total;
                                  const double D_drift_velocity_total =
                                      drift_incl_mult * tnav_div (D_num_vd * den_vd - num_vd * D_den_vd, den_vd * den_vd);
                              ...
                              const double raw_alpha_g_new = gas_superficial_velocity_input / df_safe_nonzero (new_vels.gas);
                              ...
                              double water_frac_in_liquid = 1.0 - oil_frac_in_liquid;
                                   ...
                                   double A_OW = 1.2;
                                       ...
                                       double wat_oil_interfacial_tension = fabs (gas_oil_interfacial_tension * oil_frac_in_liquid - gas_wat_interfacial_tension * water_frac_in_liquid);
                              ...
                              for (unsigned int id = 0; id < nseg_vars; ++id)
                                {
                                  ...
                                  const double arg_ow = gas_oil_interfacial_tension * oil_frac_in_liquid - gas_wat_interfacial_tension * water_frac_in_liquid;
                                  const double D_sigma_ow_total =
                                      tnav_sgn (arg_ow)
                                      * (oil_frac_in_liquid * D_sigma_go_x
                                         - water_frac_in_liquid * D_sigma_gw_x
                                         + (gas_oil_interfacial_tension + gas_wat_interfacial_tension) * D_beta_iter);
                              ...
                              const double raw_beta_o = oil_superficial_velocity_input
                              ...
                              const double Rg_iter =
                                  new_holdups.gas * new_vels.gas - gas_superficial_velocity_input;
                              const double Ro_iter =
                                  new_holdups.oil * new_vels.oil - oil_superficial_velocity_input;
                              const double Rw_iter =
                                  new_holdups.water * new_vels.water - water_superficial_velocity_input;

                              const double Rg_rel = fabs (Rg_iter) / std::max (1.0, fabs (gas_superficial_velocity_input));
                              const double Ro_rel = fabs (Ro_iter) / std::max (1.0, fabs (oil_superficial_velocity_input));
                              const double Rw_rel = fabs (Rw_iter) / std::max (1.0, fabs (water_superficial_velocity_input));

                              error = std::max (iter_delta, sqrt (Rg_rel * Rg_rel + Ro_rel * Ro_rel + Rw_rel * Rw_rel));
                    ...
                    int &dbg_inner_it)
{
  ...
  const double alpha_g_flash_init = 0.1;
  const double alpha_o_flash_init = 0.8;
  const double alpha_w_flash_init = 0.1;

  const double alpha_g_init = use_previous_df_holdup_seed ? prev_alpha_g_state : alpha_g_flash_init;
  const double alpha_o_init = use_previous_df_holdup_seed ? prev_alpha_o_state : alpha_o_flash_init;
  const double alpha_w_init = use_previous_df_holdup_seed ? prev_alpha_w_state : alpha_w_flash_init;
  ...
  const double gas_superficial_velocity_input =
      use_previous_df_holdup_seed ? (alpha_g_init * gas_phase_velocity_seed)
                                  : (alpha_g_flash_init * mixture_superficial_velocity);
  const double oil_superficial_velocity_input =
      use_previous_df_holdup_seed ? (alpha_o_init * oil_phase_velocity_seed)
                                  : (alpha_o_flash_init * mixture_superficial_velocity);
  const double water_superficial_velocity_input =
      use_previous_df_holdup_seed ? (alpha_w_init * water_phase_velocity_seed)
                                  : (alpha_w_flash_init * mixture_superficial_velocity);
  ...
  else if (fabs (alpha_l_init) > df_den_eps)
    {
  ...
  while (error > 1.e-4 && it < max_it)
    {
      ...
      {
        const double drift_den_iter =
            sqrt (tnav_div (gas_density_iter, liq_density_iter)) * temp_holdups.gas * C0_iter
            + 1.0 - temp_holdups.gas * C0_iter;

        if (fabs (drift_den_iter) > tnm::min_division)
          {
            drift_velocity_iter =
      ...
      if (fabs (new_vels.gas) > tnm::min_compare)
        raw_alpha_g_new = gas_superficial_velocity_input / new_vels.gas;
      else
      ...
      const double wat_oil_interfacial_tension_iter =
          fabs (gas_oil_interfacial_tension_iter * oil_frac_in_liquid_iter
                - gas_wat_interfacial_tension_iter * water_frac_in_liquid_iter);
      ...
      // Same beta_o update as in the main value path.
      double new_beta_o = 0.5;
      const double oil_superficial_velocity_prev = oil_superficial_velocity_input;
      if (new_holdups.liquid > tnm::min_division && fabs (new_vels.oil) > tnm::min_compare)
        {
          const double raw_beta_o =
              oil_superficial_velocity_prev / (new_holdups.liquid * new_vels.oil);
  ...
  dbg_R_g = alpha_g * gas_phase_velocity - gas_superficial_velocity_input;
  dbg_R_o = alpha_o * oil_phase_velocity - oil_superficial_velocity_input;
