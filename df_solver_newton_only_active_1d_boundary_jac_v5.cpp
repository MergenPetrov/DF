                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          const bool newton_has_water = fabs (water_superficial_velocity_input) > newton_phase_flux_eps;
                          ...
                          // Freeze the local active mode for FD only in the true interior oil+water case.
                          // For phase-boundary modes (oil-only / water-only) the production Jacobian
                          // uses a phase-appearance derivative, so the FD checker must be allowed to
                          // cross the boundary instead of being forced to the same one-phase value mode.
                          df_active_mode_for_test_function =
                              (df_newton_mode == DF_NM_COUPLED_OW) ? static_cast<int> (df_newton_mode) : -1;
                          ...
                          const char *newton_second_eq_name =
                              (df_newton_mode == DF_NM_COUPLED_OW)
                                ...
                                : ((df_newton_mode == DF_NM_OIL_ONLY) ? "ALPHA_W=0"
                                   : ((df_newton_mode == DF_NM_WATER_ONLY) ? "ALPHA_O=0" : "NONE"));
                          ...
                          auto evaluate_newton_value = [&] (double alpha_g_trial,
                                                            ...
                                                            df_local_value_state_t &S)
                            {
                              ...
                              else if (mode == DF_NM_GL_ONLY)
                                {
                                  if (!std::isfinite (ag))
                                    ag = alpha_g_flash_init;
                                  ag = std::max (simplex_eps, std::min (1.0 - simplex_eps, ag));
                                  ao = clamp01 (beta_for_gl_only) * (1.0 - ag);
                                }
                              else
                                {
                          ...
                          else if (df_newton_mode == DF_NM_GL_ONLY)
                            {
                              alpha_g_newton_only = std::max (simplex_eps, std::min (1.0 - simplex_eps, alpha_g_newton_only));
                              alpha_o_newton_only = clamp01 (beta_for_gl_only) * (1.0 - alpha_g_newton_only);
                            }
                              ...
                              for (int ils = 0; ils < 18; ++ils)
                                {
                                  ...
                                  if (df_newton_mode == DF_NM_OIL_ONLY)
                                    ...
                                    {
                                      ag_trial = std::max (simplex_eps, std::min (1.0 - simplex_eps, ag_trial));
                                      ao_trial = clamp01 (beta_for_gl_only) * (1.0 - ag_trial);
                                    }
                                  else
                                    {
                                      project_df_simplex (ag_trial, ao_trial);
                                    }
