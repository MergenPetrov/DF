if (element_status)
        {
          ...
          // Later production evaluations: use previous DF holdups and previous physical
          // signed phase superficial velocities.  They are frozen for the current Newton
          // linearization, so analytical derivatives of jmix/jsg/jso/jsw are zero in the
          // lagged branch.  test_function() receives the same frozen snapshot through a
          // scratch marker backed up/restored by df_fd_wsncs_backup_t below.
          // ----------------------------------------------------------------------------
          ...
          const double df_lagged_scratch_magic = 6.314159265358979e101;
          ...
          auto df_normalize_lagged_state = [&] (df_lagged_phase_input_state_t &st) -> bool
            {
              ...
              if (!std::isfinite (st.jsg_raw) || !std::isfinite (st.jso_raw) || !std::isfinite (st.jsw_raw))
                return false;
              ...
              if (df_lagged_input_state_valid)
                {
                  ...
                  if (nseg_vars_fd >= 5U)
                    {
                      dst->D_average_volumetric_mixture_velocity_D_seg_vars[0] = df_lagged_scratch_magic;
                      dst->D_average_volumetric_mixture_velocity_D_seg_vars[1] = df_lagged_input_state.jmix_raw;
                      dst->D_average_volumetric_mixture_velocity_D_seg_vars[2] = df_lagged_input_state.jsg_raw;
                      dst->D_average_volumetric_mixture_velocity_D_seg_vars[3] = df_lagged_input_state.jso_raw;
                      dst->D_average_volumetric_mixture_velocity_D_seg_vars[4] = df_lagged_input_state.jsw_raw;
                    }
                }
              else
                {
                  ...
                  if (seg.wsn->wsn_index == TOP_SEG_INDEX)
                    {
                      ...
                      for (unsigned int id = 0; id < nseg_vars; ++id)
                        ...
                        for (auto ic = mp.nc0; ic < mp.nc; ++ic)
                          ...
                          const bool use_frozen_df_superficial_inputs = prev_df_holdup_input_valid;
                          ...
                          const double jsg_lagged_df_input_raw = df_lagged_input_state.jsg_raw;
                          const double jso_lagged_df_input_raw = df_lagged_input_state.jso_raw;
                          const double jsw_lagged_df_input_raw = df_lagged_input_state.jsw_raw;
                          const double jmix_lagged_df_input_raw = df_lagged_input_state.jmix_raw;
                          ...
                          double average_volumetric_mixture_velocity_raw =
                              use_frozen_df_superficial_inputs
                                ? jmix_lagged_df_input_raw
                              ...
                              for (unsigned int id = 0; id < 1U + mp.nc + 1U; id++)
                                {
                                  ...
                                  if (current_seg_comp_col_inactive (id) || use_frozen_df_superficial_inputs)
                                    {
                                      // Frozen previous-Newton DF superficial velocities:
                                      // d(jmix_DF)/d(current segment vars) = 0.
                          ...
                          if (phase_inputs_from_lagged_map && use_frozen_df_superficial_inputs)
                            {
                              gas_superficial_velocity_input_raw = jsg_lagged_df_input_raw;
                              oil_superficial_velocity_input_raw = jso_lagged_df_input_raw;
                              water_superficial_velocity_input_raw = jsw_lagged_df_input_raw;
                            }
                          ...
                          for (unsigned int id = 0; id < nseg_vars; ++id)
                            {
                              ...
                              // If the phase inputs come from previous Newton DF state, both
                              // alpha_input and phase superficial velocities are frozen for this
                              // residual/Jacobian evaluation.  In that case D_jmix is already zero
                              // and D_alpha_input remains zero.  If no previous DF state exists,
                              // use the flash phase_S derivatives.
                    ...
                    int &dbg_inner_it)
{
  ...
  double jmix_lagged_df_input_raw = 0.0;
  double jsg_lagged_df_input_raw = 0.0;
  double jso_lagged_df_input_raw = 0.0;
  double jsw_lagged_df_input_raw = 0.0;
  bool prev_df_holdup_input_valid = false;
  bool use_frozen_df_superficial_inputs = false;

  // test_function() is used by finite-difference diagnostics and must evaluate the
  // same frozen lagged input as the production value/Jacobian path.  The production
  // code seeds wsncs before df_fd_wsncs_backup_t is captured; restore() then brings
  // these values back before every +/- eps call.
  {
    ...
    const double df_lagged_scratch_magic = 6.314159265358979e101;
    const unsigned int nseg_vars_local = 1U + mp.nc + 1U;
    if (prev_df_holdup_input_valid && nseg_vars_local >= 5U
        && std::isfinite (wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[0])
        && fabs (wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[0] - df_lagged_scratch_magic)
           <= 1.0e-12 * fabs (df_lagged_scratch_magic))
      {
        jmix_lagged_df_input_raw = wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[1];
        jsg_lagged_df_input_raw  = wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[2];
        jso_lagged_df_input_raw  = wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[3];
        jsw_lagged_df_input_raw  = wsncs->D_average_volumetric_mixture_velocity_D_seg_vars[4];

        const double jmix_from_phases = jsg_lagged_df_input_raw + jso_lagged_df_input_raw + jsw_lagged_df_input_raw;
        if (std::isfinite (jsg_lagged_df_input_raw)
            && std::isfinite (jso_lagged_df_input_raw)
            && std::isfinite (jsw_lagged_df_input_raw)
            && std::isfinite (jmix_from_phases))
          {
            if (!std::isfinite (jmix_lagged_df_input_raw)
                || fabs (jmix_lagged_df_input_raw - jmix_from_phases) > 1.0e-9 * std::max (1.0, fabs (jmix_from_phases)))
              jmix_lagged_df_input_raw = jmix_from_phases;
            use_frozen_df_superficial_inputs = true;
          }
      }
  }
  ...
  double average_volumetric_velocity_raw =
      (prev_df_holdup_input_valid && use_frozen_df_superficial_inputs)
        ? jmix_lagged_df_input_raw
        : average_volumetric_velocity_from_rate_raw;
